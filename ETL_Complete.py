...
def etl_dim_buch(oltp_conn, dwh_conn):
    ...
    # Query mit Denormalisierung (korrigiert - vermeidet kartesisches Produkt)
    oltp_cursor.execute("""
        SELECT
            b.buch_id,
            b.isbn,
            ...
            -- Autor denormalisiert (aus Subquery)
            autoren.autor_name,
            autoren.hauptautor_nachname,
            ...
            -- Exemplar-Anzahl (aus Subquery)
            COALESCE(exemplare.anzahl_exemplare, 0) AS anzahl_exemplare,
            COALESCE(exemplare.anzahl_verfuegbar, 0) AS anzahl_verfuegbar

        FROM BUCH b
        LEFT JOIN KATEGORIE k ON b.kategorie_id = k.kategorie_id
        LEFT JOIN VERLAG v ON b.verlag_id = v.verlag_id

        -- Subquery: Autoren aggregiert (verhindert Duplikate)
        LEFT JOIN (
            SELECT
                ba.buch_id,
            ...
            GROUP BY ba.buch_id
        ) autoren ON b.buch_id = autoren.buch_id

        -- Subquery: Exemplare aggregiert (verhindert kartesisches Produkt)
        LEFT JOIN (
            SELECT
                buch_id,
                COUNT(exemplar_id) AS anzahl_exemplare,
                COUNT(CASE WHEN status = 'Verfügbar' THEN exemplar_id END) AS anzahl_verfuegbar
            FROM BUCHEXEMPLAR
            GROUP BY buch_id
        ) exemplare ON b.buch_id = exemplare.buch_id
